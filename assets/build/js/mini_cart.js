...
if (tws__mini_cart_ul) {
  var _loop = function _loop() {
    ...
    if (currentChild.id && currentChild.nodeName === 'LI') {
      ...
      document.getElementById("tws__mini_decrement_".concat(currentChild.getAttribute('data-product_id'))).addEventListener('click', function (e) {
        // console.log(`tws__mini_decrement_${currentChild.getAttribute('data-product_id')}`);

        // action with product_id
        decrement_quantity("".concat(currentChild.getAttribute('data-product_id')));
      });
    }
    ;
  };
...
function decrement_quantity(product_id) {
  ...
  var tws__product_key = document.getElementById("tws__mini_decrement_".concat(product_id)).getAttribute('data-cart_item_key');
